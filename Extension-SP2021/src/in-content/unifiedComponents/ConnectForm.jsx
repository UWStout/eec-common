...
export default function ConnectForm (props) {
  const { userStatus, emojiList, privacy, showAffect, opened, handleClose, handleAffectChange, handleAffectClose, handleHistoryFormOpen, handleHistoryFormBack } = props
  ...
  const handleCollaborationClick = (e) => {
    ...
    if (props.handleCollaborationChange) {
      props.handleCollaborationChange(!userStatus.collaboration)
    } else {
  ...
  if (userStatus && userStatus.collaboration) {
  ...
  return (
    <div>
      ...
      <History
        opened={historyOpen} handleClose={handleHistoryClick}
        handleHistoryFormOpen={handleHistoryFormOpen}
        handleHistoryFormBack={handleHistoryFormBack}
      />
        ...
        <Paper elevation={3} className={classes.paperRoot}>
          <Grid container>
            ...
            <Grid item className={classes.firstGridRow}>
              <Button color="default" onClick={handleClose} className={classes.headerButton}>
            ...
            <Grid item className={classes.gridRow}>
              {userStatus &&
                <MoodSelect
                  handleChange={handleAffectChange} showAffect={showAffect} handleAffectClose={handleAffectClose}
                  currentAffectID={userStatus.currentAffectID} privacy={privacy} emojiList={emojiList}
                />}
              {!userStatus &&
            ...
            <Grid item className={classes.gridRow}>
              <Button color="default" onClick={userStatus ? handleCollaborationClick : () => {}} className={classes.panelButton}>
                {collaborationType}
                {'Collaboration'}
              </Button>
            </Grid>
            ...
            <Grid item className={classes.gridRow}>
              <Button
                color="default" className={classes.panelButton} onClick={() => {
                  handleHistoryClick()
                  props.handleClose()
...
ConnectForm.defaultProps = {
  privacy: { private: false, prompt: true },
  handleAffectChange: null,
  handleCollaborationChange: null,
  showAffect: false
}
