...
export default function ConnectComponent (props) {
  ...
  const getLatestUserStatus = () => {
    ...
    backgroundMessage(
      ...
      (currentUserStatus) => {
        ...
        if (emitter) { emitter.emit('userStatusChanged', currentUserStatus) }
  ...
  if (emitter) {
    ...
    emitter.on('affectInputRequested', () => {
      updateFormOpen(true)
  ...
  const affectClosed = () => {
    ...
    if (emitter) { emitter.emit('affectSurveyDone') }
  ...
  return (
    <React.Fragment>
      {PanelOpenButton}
      <ConnectForm
        userStatus={userStatus} emojiList={emojiList} privacy={privacy} opened={formOpen} showAffect={showAffect}
